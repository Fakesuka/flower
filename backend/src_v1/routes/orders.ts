...
router.post('/', validateBody(createOrderSchema), async (req, res, next) => {
  try {
    ...
    await run(
      'INSERT INTO order_events(order_id, event_type, status_to, message) VALUES (?,?,?,?)',
      [result.lastID, 'STATUS_CHANGE', 'NEW', 'Order created']
    );
...
router.post('/:id/events', requireAuth, requireRole('admin', 'florist'), validateBody(eventSchema), async (req, res, next) => {
  try {
    ...
    const result = await run(
      'INSERT INTO order_events(order_id, actor_user_id, event_type, message) VALUES (?,?,?,?)',
      [orderId, req.user!.id, req.body.event_type, req.body.message]
    );
